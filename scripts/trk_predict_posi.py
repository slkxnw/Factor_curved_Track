...
# -*- coding: utf-8 -*-
# 将trks预测结果从当前起始帧坐标系转换到当前帧坐标系，目前是基于读取的kitti官方的轨迹文件实现，后续改成基于来自slam算法定位信息实现
...
def transform_callback(req):
    ...
    if(frame_id % 1 == 0):
        rospy.loginfo("Transform cord of trks preds in frame %d and save them", frame_id)
...
def transform(args):
    ...
    eval_path = '/home/chenz/GD/dataset/KITTI/eval/pointrcnn_category_val_H1_pred/data_0'
    eval_file = open(os.path.join(eval_path, args.seqs + '.txt'), 'w')
    
    imu_pose = load_oxts_packets_and_poses(oxt_path)
    rospy.init_node('trks_pred_transform', anonymous=True)
    rospy.loginfo("Transform cord of trksand save them")
    s = rospy.Service('/trk_predict_state_store', Trk_state_store, transform_callback)
